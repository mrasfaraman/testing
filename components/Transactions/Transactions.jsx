...
export default function Trancactions({navigation}) {
  ...
  const applyFilter = () => {
    ...
    if (selectedDate) {
      filtered = filtered.filter(item => {
        ...
        const standardDateFormat = `${year}-${month.padStart(
          2,
          '0',
        )}-${day.padStart(2, '0')}`;
  ...
  useEffect(() => {
    const timeoutId = setTimeout(async () => {
      ...
      try {
        ...
        if (data?.type === 'solana') {
          let responce = await getsolTrxsignatures(
  ...
  const renderTransactions = ({item}) => {
    ...
    return (
      <TouchableOpacity
        style={[styles.transaction, {borderColor: theme.transactionBorder}]}
        onPress={() =>
          console.log(
            time,
            functionName,
        ...
        <View style={styles.row}>
          <View style={styles.left}>
            ...
            <Text style={[styles.token, {color: theme.text}]}>
              {tx?.functionName ? functionName : sendOrReceived}
            </Text>
                ...
                status === 'Success'
                  ...
                  : [
                      styles.statusYellow,
                      {
                        ...
                        borderColor: theme.statusYellow,
                      },
                    ]
              }>
              <Text
            ...
            <View style={styles.row}>
              ...
              <View>
                ...
                <View style={[styles.right, {color: theme.text}]}>
                  ...
                  <Text style={[styles.amount, {color: theme.text}]}>
                    {' '}
                    {item?.transaction_info?.symbol.toUpperCase()}
                  </Text>
                </View>
              </View>
            </View>
            <View style={styles.line}>
                    ...
                    style={
                      item?.Result === 'SuccessFinalized (MAX confirmations)'
                        ? [
                            ...
                            {color: theme.statusTextGreen},
                          ]
                        : [
                            styles.statusTextYellow,
                            {color: theme.statusTextYellow},
                          ]
                    }>
              ...
              <View style={styles.dateContainer}>
                ...
                <Text style={{color: theme.text}}>
                  {' '}
                  {item?.Timestamp.substring(10, 27)}
                </Text>
              </View>
            </View>
          </TouchableOpacity>
        ...
        {Loader ? (
          ...
          <>
            {soltrx.length == 0 && trx.length == 0 ? (
              ...
              <FlatList
                data={net == 'evm' ? trx : soltrx}
                renderItem={
                  net == 'evm' ? renderTransactions : renderSolTransactions
                }
              />
            )}
          </>

...
const MainList = ({navigation}) => {
  ...
  const {Tokens, selectedNetwork, Networks, selectedAccount, updateToken} =
    useAuth();
  ...
  const tokenevmUpdate = async (item, index) => {
    let responce = await importEVMToken(
      address.replace(/^"|"$/g, ''),
      activeNet?.nodeURL,
      activeNet?.networkName,
      item.token_address,
    );
  ...
  const tokensolUpdate = async (item, index) => {
    let responce = await importSolToken(
      address.replace(/^"|"$/g, ''),
      item.address,
    );
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      Tokens.map((item, index) => {
        let solActive =
          item?.rpc === undefined ? activeNet?.type === 'solana' : false;
        ...
        >
          ...
          <View style={styles.coinDetailOne}>
            <View style={styles.listCoinIconWrapper}>
              <Image
                style={styles.listCoinIconImg}
                source={{uri: item.logo}}
                alt="picture"
              />
            </View>
            <View style={styles.oneTextWrapper}>
              <Text style={[styles.oneTextSymbol, {color: theme.text}]}>
                {item.name}{' '}
              </Text>
          ...
          <View>{/* <Image source={TwoCoinGraph} alt="grapg" /> */}</View>
          ...
          <View>
            ...
            <Text style={[styles.thirdCoinListCrypto, {color: theme.text}]}>
              {' '}
    ...
    <View style={styles.mainWrapper}>
      <View style={styles.mainListHeader}>
        <TouchableOpacity
          ...
          <Text style={[styles.listTabText, {color: theme.text}]}>Assets</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[
            styles.listTab,
          ...
          <Text style={[styles.listTabText, {color: theme.text}]}>History</Text>
        </TouchableOpacity>
      </View>
      {pageSwitch == 'one' && (
        <View>
